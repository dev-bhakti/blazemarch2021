    ...
    constructor(){}
     ...
     DisplayTable(data,Delete_Row,Edit_Row){
        ...
        for(var head = 0; head < headers.length; head++){
          table+= `<th >${headers[head]}</th>`; 
            //  table+= `<th onclick= 'sortTable(0)'>${headers[head]}</th>`; 
            // table+= "<th>"+headers[head]+ "<br><span style='cursor: pointer;' onclick= 'sortTable(0)'>&#9650 </span>" +"</th>";
        } 
        ...
        for(let row = 0; row < data.length; row++){
            ...
            for(var val = 1 ; val<headers.length;val++){
                ...
                table+= `<td value =${data[row][headers[val]]}>
                <span id='sp-${row}-${val}'>${data[row][headers[val]]}</span> 
            ...
            table+= `<td id='row${row}-${val}'><button class="btn btn-success" id='E-${row}${val}' onclick=EditRow(${prodData},'${row}','${val}')>Edit</button>
            </td>`
            table+= `<td><button class="btn btn-danger" id='${row}${val}' onclick='DeleteRow(${row},${prodData})'>Delete</button>
            </td>`
            table+=`</tr>`;           
        }
...
for( let i=1;i<row2;i++){
    ...
    `<button class="btn btn-info" onclick='Edit(${JSON.stringify(data)},${row1},${row2})'>Update</button>
...
function Edit(data,row,col){
    ...
    for(let e = 1; e < headers.length; e++){
        span=`sp-${row}-${e}`
        input=`${row}-${e}`
        let i=`${row}`
        data[i][headers[e]]=document.getElementById(input).value
        document.getElementById(input).style.display='none'
        document.getElementById(span).innerHTML=document.getElementById(input).value
        document.getElementById(span).style.display='block'
        document.getElementById(edit).innerHTML=
        `<button class="btn btn-success" id='${row}-${col}' onclick=EditRow(${JSON.stringify(data)},${row},${col})>Edit</button>`
...
function DeleteRow(delte_row,data){
  data.splice(delte_row)
  document.getElementById(delte_row).innerHTML=''
}
